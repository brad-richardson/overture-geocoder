...
__export(index_exports, {
  ...
  readByBbox: () => import_overturemaps2.readByBbox,
  readByBboxAll: () => import_overturemaps2.readByBboxAll,
...
var OvertureGeocoder = class {
   ...
   */
  async close() {
    await (0, import_overturemaps.closeDb)();
  }
  // ==========================================================================
  ...
  async getNearbyPlaces(lat, lon, options = {}) {
    ...
    try {
      const features = await (0, import_overturemaps.readByBboxAll)("place", bbox, { limit: limit * 3 });
  ...
  async getNearbyAddresses(lat, lon, options = {}) {
    ...
    const bbox = this.radiusToBbox(lat, lon, radiusKm);
    try {
      const features = await (0, import_overturemaps.readByBboxAll)("address", bbox, { limit: limit * 3 });
      return features.filter((f) => f.geometry.type === "Point").map((f) => {
        const props = f.properties;
        const coords = f.geometry.coordinates;
        const fLon = coords[0];
