...
# The marker/inventory/dictionary correctness protocol lives in one module so
# every phase goes through a single verified entry point instead of re-deriving
# the "data before marker" ordering, the content-addressing, and the
# fail-closed validation. These names are imported (and re-exported) here so
# build_id_index stays the import surface its consumer scripts already use.
from id_index_protocol import (  # noqa: F401
    ID_INDEX_FORMAT_VERSION,
    ID_INVENTORY_DIR,
    ID_INVENTORY_SET_VERSION,
    ID_LOCATOR_MANIFEST,
    ID_STAGE_INVENTORY_VERSION,
    TRANSIENT_DUCKDB_ERRORS,
    TYPE_THEME_MAP,
    TYPE_THEME_MAP_VERSION,
    _build_marker_range,
    _build_marker_shard_inventory,
    _canonical_json_bytes,
    _dictionary_reference,
    _glob_files,
    _inventory_set_reference,
    _inventory_set_sha256,
    _is_transient,
    _load_inventory_set,
    _load_locator_dictionary_binding,
    _load_locator_dictionary_reference,
    _load_locator_manifest_and_dictionary,
    _load_stage_inventory,
    _locator_dictionary_marker_reference,
    _make_inventory_set,
    _make_locator_dictionary,
    _make_stage_inventory,
    _marker_is_current,
    _patch_update_build_markers,
    _prefix_in_build_range,
    _publish_inventory_set,
    ...
    _require_locator_input_inventory_set_sha,
    _retry_transient,
    _sha256_json,
    _shard_marker_entry,
    _stage_inventory_reference,
    _sum_build_marker_records,
    _type_theme_metadata,
    _upload_to_r2,
    _validate_build_marker_dictionary_sha,
    _validate_inventory_set,
    _validate_inventory_set_reference,
    _validate_locator_dictionary,
    _validate_locator_manifest,
    _validate_source_filename,
    _validate_stage_inventory,
    _validate_stage_inventory_reference,
    _write_staging_marker,
    reconcile_build_release_staging,
)
...
def phase_build_r2(prefix_len, r2_config, version, release_version, workers, prefixes=None,
    ...
    # The raw staging glob above trusts whatever release files exist. Before
    # any of them can flow into shards, prove the per-type _SUCCESS markers
    # still stand behind exactly this file set (fail closed on either a
    # marker without files or files without a current marker). Reconcile the
    # unfiltered discovery: bucket filtering below intentionally drops other
    # ranges' files and must not masquerade as missing staging.
    reconcile_build_release_staging(r2_config, version, release_files)
