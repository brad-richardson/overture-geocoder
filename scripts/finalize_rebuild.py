...
def inventory_global_v2_slice(client, *, version: str) -> dict:
    """Stream-hash every object currently present under one family slice.

    This is intentionally disk-bounded: ``R2Client.object_identity`` downloads
    and hashes one object in a temporary directory, then removes it before the
    next key. The resulting listing is the small control-plane input consumed by
    ``global_v2_executor.build_slice_manifest``; no workflow job recursively
    downloads the planet family fleet.
    """

    _slice_version_key(version)
    prefix = f"{version}/"
    objects = []
    for key in sorted(client.list_prefix(prefix)):
        if not key.startswith(prefix):
            raise ValueError(f"R2 listing escaped the slice prefix: {key}")
        size, digest = client.object_identity(key)
        if size <= 0 or not SHA256_RE.fullmatch(digest):
            raise ValueError(f"R2 object has an invalid identity: {key}")
        objects.append({"object_key": key, "bytes": size, "sha256": digest})
    return {
        "schema": "overture-global-v2-remote-listing-v1",
        "slice_version": version,
        "objects": objects,
    }


def publish_global_v2_slice(
    client,
    *,
    request_path: Path,
    contract_path: Path,
    runtime_path: Path,
    addresses_manifest_path: Path,
    places_manifest_path: Path,
    phase_evidence_path: Path,
    candidate_path: Path,
    log=print,
) -> dict:
    """Publish the non-promoting slice completion marker create-only and last.

    The candidate is rebuilt from a fresh streaming remote listing immediately
    before publication. Every family object, manifest, size, SHA-256, required
    entrypoint, family total, and producer phase is therefore proven before
    ``slice-manifest.json`` becomes visible. An identical existing marker makes
    an interrupted/retried finalizer idempotent; a differing marker or any
    changed family object fails closed. No catalog key is read or written.
    """
    ...
    address_manifest = _load_json(addresses_manifest_path)
    places_manifest = _load_json(places_manifest_path)
    phase_evidence = json.loads(phase_evidence_path.read_text())
    version = request.get("slice_version")
    _slice_version_key(version)
    marker_key = f"{version}/slice-manifest.json"

    def publish_remote_verify(candidate_bytes: bytes, candidate: dict) -> dict:
        verified = candidate.get("verified_version_objects")
        if not isinstance(verified, list) or not verified:
            raise ValueError("slice candidate has no verified family object set")
        total_bytes = 0
        for item in verified:
            if (
                ...
                raise ValueError("slice candidate contains invalid verified object evidence")
            total_bytes += item["size_bytes"]
        candidate_sha256 = hashlib.sha256(candidate_bytes).hexdigest()
        report = {
            "schema": "overture-global-v2-remote-verification-v1",
            "slice_version": version,
            "verified_objects": len(verified),
            "verified_bytes": total_bytes,
            "slice_manifest_candidate_sha256": candidate_sha256,
            "exact_family_object_set": True,
            "streaming_content_identity_verified": True,
            "catalog_published": False,
            "legacy_core_rebuilt": False,
            "divisions_rebuilt": False,
        }
        report_bytes = executor.canonical_json(report)
        report_key = f"{contract['namespace']['immutable_root']}/remote-verify/report.json"
        report_artifact = executor.normalize_artifact(
            {
        ...
        dependencies = [
            item for item in phase_evidence if item.get("phase") in {"reduce", "head"}
        ]
        completion_key = f"{contract['namespace']['phase_completion_root']}/remote-verify.json"
        completion_runtime = runtime
        existing_completion = None
        if completion_key in set(client.list_prefix(completion_key)):
            existing_completion_bytes = client.get_object(completion_key)
            existing_completion = json.loads(existing_completion_bytes)
            completion_runtime = existing_completion.get("runtime")
        completion = executor.build_control_phase_completion(
            contract,
            completion_runtime,
            phase="remote-verify",
        ...
        completion_bytes = executor.canonical_json(completion)
        if existing_completion is not None and existing_completion != completion:
            raise ValueError("existing remote-verification completion differs")
        # The report is immutable proof of the one-pass listing. Its signed
        # completion marker is create-only and last within the phase; the slice
        # marker is published only after both readbacks succeed.
        client.put_immutable(report_key, report_bytes)
        if client.object_identity(report_key) != (
            len(report_bytes), hashlib.sha256(report_bytes).hexdigest()
        ):
            raise ValueError("remote-verification report differs after publication")
        client.put_immutable(completion_key, completion_bytes)
        if client.object_identity(completion_key) != (
            len(completion_bytes), hashlib.sha256(completion_bytes).hexdigest()
        ):
            raise ValueError("remote-verification completion differs after publication")
        return {
    ...
    def publish_finalize_slice(
        marker_identity: tuple[int, str], remote_verify: dict
    ) -> dict:
        artifact = executor.normalize_artifact(
            {
                "object_key": marker_key,
                "bytes": marker_identity[0],
                "sha256": marker_identity[1],
            },
            contract,
        )
        completion_key = (
        ...
        existing_completion = None
        if completion_key in set(client.list_prefix(completion_key)):
            existing_bytes = client.get_object(completion_key)
            existing_completion = json.loads(existing_bytes)
            completion_runtime = existing_completion.get("runtime")
        completion = executor.build_control_phase_completion(
            contract,
            completion_runtime,
            phase="finalize-slice",
            artifacts=[artifact],
            dependency_evidence=[{
                "phase": "remote-verify",
                "completion_sha256": remote_verify["completion_sha256"],
            }],
            details={
                "slice_version": version,
                "slice_manifest_key": marker_key,
                "slice_manifest_sha256": marker_identity[1],
                "non_promoting": True,
                "catalog_published": False,
            },
        )
        if existing_completion is not None and existing_completion != completion:
            raise ValueError("existing finalize-slice completion differs")
        completion_bytes = executor.canonical_json(completion)
        client.put_immutable(completion_key, completion_bytes)
        if client.object_identity(completion_key) != (
            len(completion_bytes), hashlib.sha256(completion_bytes).hexdigest()
        ):
            raise ValueError("finalize-slice completion differs after publication")
        return {
            "completion_key": completion_key,
            "completion_sha256": completion["completion_sha256"],
            "slice_manifest_sha256": marker_identity[1],
    ...
    # A completed retry does not re-download the planet. Rebuild the candidate
    # from its declared identities plus the independently retained family
    # manifests, prove the remote key set is exact, and stream-check only the
    # completion marker. The first completion path below still performs one
    # fresh streaming hash of every family object before marker publication.
    current_keys = set(client.list_prefix(f"{version}/"))
    if marker_key in current_keys:
        if candidate_path.is_file():
            candidate = _load_json(candidate_path)
        else:
            candidate_path.parent.mkdir(parents=True, exist_ok=True)
            candidate_path.write_bytes(client.get_object(marker_key))
            candidate = _load_json(candidate_path)
        runtime = executor.validate_runtime_for_contract(candidate.get("runtime"), contract)
        verified = candidate.get("verified_version_objects")
        if not isinstance(verified, list):
            raise ValueError("existing slice candidate has no verified object set")
        objects = []
        for item in verified:
            if not isinstance(item, dict) or not isinstance(item.get("href"), str):
                raise ValueError("existing slice candidate object is invalid")
            relative = item["href"].removeprefix("./")
            objects.append(
                {
                    "object_key": f"{version}/{relative}",
        ...
        rebuilt = executor.build_slice_manifest(
            contract,
            request,
            runtime,
            address_manifest=address_manifest,
            places_manifest=places_manifest,
            remote_listing=candidate_listing,
            phase_evidence=phase_evidence,
        )
        candidate_bytes = executor.canonical_json(rebuilt)
        if candidate_path.read_bytes() != candidate_bytes:
            raise ValueError("existing slice candidate differs from verified manifests")
        remote_verify = publish_remote_verify(candidate_bytes, rebuilt)
        expected_keys = {item["object_key"] for item in objects} | {marker_key}
        if current_keys != expected_keys:
            raise ValueError("completed slice remote key set differs")
        expected_marker_identity = (
            len(candidate_bytes),
            hashlib.sha256(candidate_bytes).hexdigest(),
        )
        if client.object_identity(marker_key) != expected_marker_identity:
            raise ValueError("existing slice completion marker differs")
        finalize_slice = publish_finalize_slice(expected_marker_identity, remote_verify)
        log(f"Slice completion marker already exists and is identical: {marker_key}")
        return {
            "slice_version": version,
            "slice_manifest_key": marker_key,
            "slice_manifest_sha256": expected_marker_identity[1],
            "remote_verify": remote_verify,
            "finalize_slice": finalize_slice,
            "verified_family_objects": len(objects),
            "catalog_published": False,
            "legacy_core_rebuilt": False,
            "divisions_rebuilt": False,
        }

    listed = inventory_global_v2_slice(client, version=version)
    marker_objects = [
        item for item in listed["objects"] if item["object_key"] == marker_key
    ]
    if len(marker_objects) > 1:
        raise ValueError("slice inventory contains duplicate completion markers")
    family_listing = {
        **listed,
        "objects": [
            item for item in listed["objects"] if item["object_key"] != marker_key
    ...
    candidate = executor.build_slice_manifest(
        ...
        address_manifest=address_manifest,
        places_manifest=places_manifest,
        remote_listing=family_listing,
        phase_evidence=phase_evidence,
    )
    candidate_bytes = executor.canonical_json(candidate)
    if candidate_path.is_file():
        if candidate_path.read_bytes() != candidate_bytes:
            raise ValueError("slice candidate differs from fresh remote verification")
    else:
        # The fresh streaming inventory above is the one whole-slice readback.
        # Persist its deterministic candidate locally rather than requiring a
        # caller to perform an identical pre-pass merely to construct this file.
        candidate_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    if marker_objects:
        raise AssertionError("marker presence should have taken the completed retry path")
    remote_verify = publish_remote_verify(candidate_bytes, candidate)
    # Immutable family data/manifests and the remote-verification completion
    # were verified above. Publish the slice marker last; put_immutable is
    # create-only and verifies its readback.
    client.put_immutable(marker_key, candidate_bytes)

    expected_after_keys = {
        item["object_key"] for item in family_listing["objects"]
    } | {marker_key}
    if set(client.list_prefix(f"{version}/")) != expected_after_keys:
        raise ValueError("slice object set changed during completion publication")
    if client.object_identity(marker_key) != expected_marker_identity:
        raise ValueError("slice marker identity changed after publication")
    finalize_slice = publish_finalize_slice(expected_marker_identity, remote_verify)
    log(
        f"Published non-promoting slice marker last: {marker_key} "
        f"({len(family_listing['objects'])} verified family objects)"
    )
    return {
        "slice_version": version,
        "slice_manifest_key": marker_key,
        ...
        "remote_verify": remote_verify,
        "finalize_slice": finalize_slice,
        "verified_family_objects": len(family_listing["objects"]),
        "catalog_published": False,
        "legacy_core_rebuilt": False,
        "divisions_rebuilt": False,
    }
...
def main() -> None:
    ...
    global_v2_inventory_p = subparsers.add_parser("inventory-global-v2-slice")
    global_v2_inventory_p.add_argument("--version", required=True)
    global_v2_inventory_p.add_argument("--output", type=Path, required=True)
    global_v2_inventory_p.add_argument("--base-url", default=None)
    global_v2_inventory_p.add_argument("--endpoint", default=None)
    global_v2_inventory_p.add_argument("--bucket", default=None)

    publish_global_v2_p = subparsers.add_parser("publish-global-v2-slice")
    publish_global_v2_p.add_argument("--request", type=Path, required=True)
    publish_global_v2_p.add_argument("--contract", type=Path, required=True)
    publish_global_v2_p.add_argument("--runtime", type=Path, required=True)
    publish_global_v2_p.add_argument(
        "--addresses-manifest", type=Path, required=True
    )
    publish_global_v2_p.add_argument("--places-manifest", type=Path, required=True)
    publish_global_v2_p.add_argument("--phase-evidence", type=Path, required=True)
    publish_global_v2_p.add_argument("--candidate", type=Path, required=True)
    publish_global_v2_p.add_argument("--report", type=Path, required=True)
    publish_global_v2_p.add_argument("--base-url", default=None)
    publish_global_v2_p.add_argument("--endpoint", default=None)
    publish_global_v2_p.add_argument("--bucket", default=None)
    ...
    if args.command == "publish-family":
        ...
        try:
            ...
            sys.exit(1)
        return
    if args.command == "inventory-global-v2-slice":
        client = _build_client_from_env(args)
        try:
            listing = inventory_global_v2_slice(client, version=args.version)
        except (PromotionError, ValueError) as exc:
            print(f"::error::{exc}", file=sys.stderr)
            sys.exit(1)
        args.output.parent.mkdir(parents=True, exist_ok=True)
        args.output.write_bytes(_family_canonical_json(listing))
        print(
            f"Stream-verified {len(listing['objects'])} objects under "
            f"{args.version}/"
        )
        return
    if args.command == "publish-global-v2-slice":
        client = _build_client_from_env(args)
        try:
            report = publish_global_v2_slice(
                client,
                request_path=args.request,
                contract_path=args.contract,
                runtime_path=args.runtime,
                addresses_manifest_path=args.addresses_manifest,
                places_manifest_path=args.places_manifest,
                phase_evidence_path=args.phase_evidence,
        ...
        args.report.write_bytes(_family_canonical_json(report))
        return
